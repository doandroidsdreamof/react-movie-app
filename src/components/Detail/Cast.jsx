...
const Cast = () => {
  ...
  return (
   ...
   <div className="ml-3">
     <p className="font-medium text-gray-200">{actor?.original_name}</p>
     <p className="text-sm text-gray-300   line-clamp-2">character {actor?.character.length > 20 ? actor?.character.substring(0, 20) + "..." : actor?.character}</p>
   </div>
   </div>
      ))}
    </>
